...
- 评估协议：subject-level 5-fold × 10 repeats，repeat offset=10。
- 总体结论：PASS。
- 审计范围：逐条预测验证被试属于对应外层 test，且不属于同一 split 的 train/val。

## `auto_signal_pca_crossfit10_offset10_predictions.csv`
- 行数：400 / 期望 400。
...
## `auto_signal_pca_subspace15_crossfit10_offset10_predictions.csv`
- 行数：400 / 期望 400。
- 唯一被试：40；每人恰好 10 条预测的被试数：40。
- subject-repeat 重复行：0。
- split 成员关系违规：0。
